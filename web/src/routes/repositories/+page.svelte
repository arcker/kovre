<script lang="ts">
	...
	async function onInit(name: string) {
		...
		try {
			...
			banner = res.justInitialized
				? `Initialized repository "${name}" (rustic config + keys + empty index written on disk).`
	...
	async function onDelete(name: string) {
		...
		if (!confirm(`Delete repository "${name}"? The rustic data on disk is NOT touched.`)) return;
...
<p class="lead">
	A repository is a rustic backend (a folder, a UNC share, …) plus the password file used to
	unlock it. Each job points at one of these by name.
</p>
				...
				<tr>
					...
					<td class="actions">
						{#if status[name]?.initialized}
							<span class="ready" title="rustic config exists at <path>/config">✓ initialized</span>
						{:else}
							<button
								type="button"
								class="init"
								...
								onclick={() => onInit(name)}
								title="Materialize the rustic config on disk"
							>
								{busyName === name ? '…' : 'init'}
							</button>
						{/if}
